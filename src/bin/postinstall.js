...
function getCursorConfigPaths() {
    ...
    const possiblePaths = [];
    if (process.platform === 'win32') {
        possiblePaths.push(path.join(process.env.APPDATA || '', 'Cursor', 'User', 'globalStorage', 'rooveterinaryinc.roo-cline', 'settings', 'cline_mcp_settings.json'), path.join(homeDir, '.cursor', 'mcp.json'), path.join(process.env.APPDATA || '', 'cursor', 'mcp.json'));
    }
    else if (process.platform === 'darwin') {
        possiblePaths.push(path.join(homeDir, 'Library', 'Application Support', 'Cursor', 'User', 'globalStorage', 'rooveterinaryinc.roo-cline', 'settings', 'cline_mcp_settings.json'), path.join(homeDir, '.cursor', 'mcp.json'), path.join(homeDir, 'Library', 'Application Support', 'Cursor', 'mcp.json'));
    }
    else {
        // Linux
        possiblePaths.push(path.join(homeDir, '.config', 'Cursor', 'User', 'globalStorage', 'rooveterinaryinc.roo-cline', 'settings', 'cline_mcp_settings.json'), path.join(homeDir, '.cursor', 'mcp.json'), path.join(homeDir, '.config', 'cursor', 'mcp.json'));
    }
...
function main() {
    ...
    if (claudeSuccess || cursorSuccess) {
        console.log('\n🎉 Setup completed!');
        if (claudeSuccess)
            console.log('📱 Restart Claude Desktop to use the MCP server');
        if (cursorSuccess)
            console.log('💻 Restart Cursor to use the MCP server');
    }
    else {
